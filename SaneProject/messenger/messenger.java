...
import moticon.networks.ForwardingTable;
import moticon.networks.LL3PDaemon;
import moticon.networks.RouteTableEntry;
...
public class Messenger implements Runnable {
	...
	public void getObjectReferences(Factory factory){
		...
		layer3Daemon = factory.getLL3PDaemonReference(); // layer 3 daemon, used to send messages.
	...
	private void resetSpinner(){
		...
		for (RouteTableEntry entry : validNetworkList){
			if (entry.getNetwork().compareTo(-1) > 0 && entry.getNetwork().compareTo(16) < 0)  
					spinnerAdapter.add(allNamesAndNetworks.get(entry.getNetwork()-1)); // network numbers go from 1-15, array elements from 0-14
		}
